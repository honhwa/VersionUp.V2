...
namespace VersionUp.V2
{
    ...
    internal sealed class ContextMenu
    {
        ...
        public const int CommandBuildM0 = 0x0500;
        public const int CommandBuildMm = 0x0600;
        ...
        public async Task InitializeMenuAsync(AsyncPackage package)
        {
            ...
            if (commandService != null)
            {
                ...
                var menuCommandBuildM0 = new CommandID(CommandSet, CommandBuildM0);
                var menuBuildM0 = new MenuCommand(this.MenuBuildCallbackM0, menuCommandBuildM0);
                commandService.AddCommand(menuBuildM0);

                var menuCommandBuildMm = new CommandID(CommandSet, CommandBuildMm);
                var menuBuildMm = new MenuCommand(this.MenuBuildCallbackMm, menuCommandBuildMm);
                commandService.AddCommand(menuBuildMm);
            }
        }
        ...
        private void UpVersion(VersionTypes versionType)
        {
            ...
            if (selProject != null)
            {
                try
                {
                    var assemblyVersion = new Version(selProject.Properties.Item("AssemblyVersion").Value.ToString());
                    ...
                    switch (versionType)
                    {
                        case VersionTypes.M0:
                            newAssemblyVersion = $"{dt:yy}.{dt:MM}.{dt:dd}.{dt:HH}{dt.ToString("mm").Substring(0, 1)}0";
                            break;

                        case VersionTypes.Mm:
                            newAssemblyVersion = $"{dt:yy}.{dt:MM}.{dt:dd}.{dt:HHmm}";
                            break;
                        default:
                            switch (versionType)
                            {
        ...
        private void MenuBuildCallbackM0(object sender, EventArgs e)
        {
            ThreadHelper.ThrowIfNotOnUIThread();

            this.UpVersion(VersionTypes.M0);
        }
        private void MenuBuildCallbackMm(object sender, EventArgs e)
        {
            ThreadHelper.ThrowIfNotOnUIThread();

            this.UpVersion(VersionTypes.Mm);
        }
    }
}
